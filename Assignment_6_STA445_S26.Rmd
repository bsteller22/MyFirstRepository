...
```{r}
(GY1 <- GY %>%
  mutate(Year = fct_relevel(Year, "FirstYear", "Sophomore",
                            "Junior", "Senior")))
```
...
```{r}
(B %>%
  arrange(match(First.Name, A$Name)) %>%
  cbind(A) %>%
  select(-Name))
```
...
```{r}
(join_cmnd <- A %>%
  left_join(B, by = c("Name" = "First.Name")))
```
...
```{r}
(cmbnd_gp <- alice_gp%>%
   full_join(A, by = c("First.Name" = "Name")))
```
...
```{r}
(Flag_Temp <- read_csv("https://raw.githubusercontent.com/BuscagliaR/STA_444_v2/master/data-raw/FlagMaxTemp.csv"))
```
...
Flag_Temp %>%
  ...
  geom_line()
     labs(
    title = "Monthly Avg Max Temp 2013-15",
    x = "Date",
    y = "Temperature"
     )
...
flights %>%
  ...
  inner_join(airports, b = c("dest" = "faa"))
...
(Derek <- Customers %>%
  ...
  inner_join(Cards, by = c("PersonID")) %>%
  inner_join(Transactions, by = c("CardID")) %>%
  inner_join(Retailers, by = c("RetailID"))) %>%
...
(Aubrey <- Customers %>%
   ...
   inner_join(Cards, by = c("PersonID")) %>%
   select(CardID, PersonID))
cardID <- Aubrey[[1]]
personID <- Aubrey[[2]]
Cards %>%
  mutate(Exp_DateTime = if_else(CardID == cardID, ymd_hms("2019-10-15 4:28:21"), 
                                Exp_DateTime)) %>%
  add_row(CardID = "0409030923011111", PersonID = personID, 
          Issue_DateTime = Sys.Date(), 
...
```{r}
card <- "0409030923011111"
retailid <- 1
datetime <- ymd_hms("2019-10-16 02:30:21")
amount <- 4.98
Transactions %>%
  add_row(CardID = card, RetailID = retailid, DateTime = datetime, 
          Amount = amount)
```
